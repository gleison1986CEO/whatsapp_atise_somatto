...
export const scheduleServiceMonitor = new Queue("ScheduleServiceMonitor", connection);
...
export const sendServiceScheduleMessages = new Queue(
  "SendServiceSacheduledMessages",
  connection
);
...
async function handleVerifyServiceSchedules(job) {
  try {
    logger.info("VerifyService Schedules -> SendServiceSchedules -> 0711");
...
export async function startQueueProcess() {
  ...
  scheduleServiceMonitor.process("VerifyServiceShedules", handleVerifyServiceSchedules);

  sendScheduledMessages.process("SendMessage", handleSendScheduledMessage);

  sendServiceScheduleMessages.process("SendSheduleMessage", handleSendServiceScheduledMessage);
  ...
  scheduleServiceMonitor.add(
    "VerifyServiceShedules",
    {},
    {
      repeat: { cron: "*/5 * * * * *" },
      removeOnComplete: true
